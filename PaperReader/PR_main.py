...
def main():
    ...
    for idx, pdf_path in enumerate(progress_bar, 1):
        ...
        try:
            ...
            spinner_thread = threading.Thread(target=analysis_spinner, args=(progress_bar, idx, len(pdf_files), stop_event))
            ...
            if analysis is None:
                print(Fore.RED + "\n分析失败，结果为空" + Style.RESET_ALL, flush=True)
            ...
            print(Fore.RED + f"\n处理 {file_name} 时出错: {e}" + Style.RESET_ALL, flush=True)
            # 记录错误文件
